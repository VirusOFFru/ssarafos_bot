...
async def create_order(user_id: int, username: str, full_name: str, address: str, total: int, items: list[dict]) -> int:
    async with aiosqlite.connect(DB_PATH) as db:
        cursor = await db.execute(
            "INSERT INTO orders (user_id, username, full_name, address, total) VALUES (?, ?, ?, ?, ?)",
            (user_id, username, full_name, address, total),
        )
